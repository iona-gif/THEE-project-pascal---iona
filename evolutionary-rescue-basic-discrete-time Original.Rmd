...
simulate_one_gen <- function(N_AA, N_Aa, N_aa, w_AA, w_Aa, w_aa, mut_rate) {
  ...
  offsp_AA <- rpois(1, N_AA * w_AA)
  offsp_Aa <- rpois(1, N_Aa * w_Aa)
  offsp_aa <- rpois(1, N_aa * w_aa)
  
  # Gesamtzahl der Gameten proportional zur Häufigkeit der Allele A und a
  total_gametes <- 2 * (offsp_AA + offsp_Aa + offsp_aa)
  freq_A <- (2 * offsp_AA + offsp_Aa) / total_gametes
  freq_a <- 1 - freq_A
  
  # Mutation (A -> a und a -> A)
...
>>>>>>> 96dd4a512028f7873ba93d5c4843b6603ab91630
...
simulate_gen <- function(N_AA, N_Aa, N_aa, w_AA, w_Aa, w_aa) {
  ...
  # 1 = p^2 + 2pq + q^2
  predicted_frequency_AA <- (freq_A / total_gametes)^2
  predicted_frequency_aa <- (freq_a / total_gametes)^2
  predicted_frequency_aA <- 2*(freq_A / total_gametes)*(freq_a / total_gametes)
