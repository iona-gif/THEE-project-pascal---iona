...
Idea: Inbreed depression in population with rezessiv disease (aa) that leads to lower fitness and is therefore declining in population size. Goal is to rescue population with hybrids. How many healthy individuals are needed to rescue the population. only healthy (AA) are added, 
Assumptions:
  - starting population is homozygot for the disease (aa)
  - healthy added individuals are homozygot without the disease (AA)
  - second generation can mate with sick individuum or hybrid
...
Parameters we need:
  - initial Population size with genotype aa -> N0_aa
  - initial Population size with genotype AA -> N0_AA
  - fitness disadvantage of heterozygotes aa -> decay.rate_aa
  - 
Variables we need:
  ...
  - amount of different genotype -> aa, aA, AA
  - new generation = offspring & new gained same genotype, -lost genotype, fitness disadvantage added 
...
<<<<<<< HEAD
# --- Simulation einer Generation mit drei Genotypen ---
simulate_gen <- function(N_AA, N_Aa, N_aa, w_AA, w_Aa, w_aa) {
=======
# --- Simulation of generation with three genotypes ---
simulate_one_gen <- function(N_AA, N_Aa, N_aa, w_AA, w_Aa, w_aa, mut_rate) {
>>>>>>> 2daf5c81ce54e410788aa3b743f2b9b1b37a0938
  # Effektive Fortpflanzung (unter Berücksichtigung von decay_rate und Fitness)
  ...
  # Change (A -> a und a -> A); p = (freq_A / total_gametes), q = (freq_a / total_gametes)
  # 1 = p^2 + 2pq + q^2
  ...
  #freq_A_mut <- freq_A * (1 - mut_rate) + freq_a * mut_rate
  #freq_a_mut <- 1 - freq_A_mut
  
  # Random draw of new genotypes
  ...
  # Zufällige Neukombination (Hardy-Weinberg nach Selektion & Mutation)
  #N_total <- offsp_AA + offsp_Aa + offsp_aa
  #N_AA_new <- rbinom(1, N_total, prob = freq_A_mut^2)
  #N_Aa_new <- rbinom(1, N_total - N_AA_new, prob = 2 * freq_A_mut * freq_a_mut / (1 - freq_A_mut^2))
  #N_aa_new <- N_total - N_AA_new - N_Aa_new
  
  return(c(N_AA_new, N_Aa_new, N_aa_new))
}

print(simulate_gen(N_AA, N_Aa, N_aa, w_AA, w_Aa, w_aa, mut_rate))
```

#Notes First we define the variables and parameters. Parameters we need: - initial Population size with genotype aa -\> N0_aa - initial Population size with genotype AA -\> N0_AA - fitness disadvantage of heterozygotes aa -\> decay.rate_aa - Variables we need: - we dont have a mutation rate but a probability of aa becoming aA (dencity dependence) - probability of aA becoming AA (dencity dependence) - amount of different genotype -\> aa, aA, AA - new generation = offspring & new gained same genotype, -lost genotype, fitness disadvantage added

Functions we need: - Function that calculates probabillity for turning into different genotype - p(Aa= ) - g1 -\> \<90 aa, 10 Aa - g2 -\>
...
```{r}
simulate_gen <- function(N_a, N_A, decay_rate, sel_coeff, mut_rate) {
