...
simulate_gen <- function(N_aa, N_aA, N_AA, r_aa, s_aA, s_AA) {
  ...
  # With the predicted frequencies we are forming the genotype frequencies of the offspring folowing the expectaions of the Hardy-Weinberg equillibrium.
  predicted_freq_aa <- (freq_a/total_gametes)^2 # = p^2 
  predicted_freq_aA <- 2*(freq_A / total_gametes)*(freq_a / total_gametes) # = 2pq
  predicted_freq_AA <- (freq_A/total_gametes)^2 # = q^2
...
## Create a plot for all of our data
...
# Similar to the previous rescue probability function, but now includes the calculation of the final allele frequency of "a" in each replicate.
...
# Convert NAs back to zeros (this is due to an error that came up if we didn't do so)
...
# same function/approach as used in the previous step (Allele frequency (a) vs. introduced number of individuals (N_AA))
...
rescue_prob_aa <- function(N_aa_intro, n_reps, N_AA,
  ...
  call_freq_a  <- numeric(n_reps) # vector to store allele frequencies for each replicate
