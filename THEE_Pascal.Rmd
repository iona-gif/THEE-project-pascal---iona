...
data_table_plot <- mean_comb[mean_comb$s_aA_fit_hybrid_results==1 & mean_comb$N_AA_str_pop_results==5,]
ggplot(data_table_plot, aes(x= N_aa_str_pop_results, y= N_aA_results))+geom_point()

ggplot(mean_comb, aes(x= N_aa_str_pop_results, y= N_aA_results))+geom_point()



 
ggplot(mean_comb, aes(x= N_aa_str_pop_results, y=N_aA_results)) + geom_point() +
  facet_wrap(vars(cols=factor(s_aA_fit_hybrid_results), rows=factor(N_AA_str_pop_results)))
...
long_dat <- mean_comb %>%
  pivot_longer(cols = c(N_aA_results, N_AA_results),   # beide y-Größen
               names_to = "variable",
               values_to = "value")
