...
plot(N_AA_values, rescue_probs, pch = 20,
     ...
     main = "Evolutionary Rescue Probability vs. Introduced Individuals")
...
to show the effect of time when AA gets introduced: adjust initial pop. size of aa (rename also in plot)
...
#Here I transform the returned array (call_freq_a) into a data.frame where we have now 2 columns (1.=N_AA, 2.= allele freq. of a n_reps)
...
ggplot(full_array, aes(x = N_AA, y = allele.freq)) +
  ...
  scale_x_continuous(breaks = c(0:10)) +         # benutzerdefinierte x-Achsenticks
  labs(
...
this still got an error that need to be fixed 

```{r}
# --- Rescue probability simulation ---
n_reps <- 100

rescue_prob_aa <- function(N_aa_intro, N_AA = 10, n_reps = 100, 
                        N_aA = 0, 
                        r_aa = 0.2, s_aA = 1.10, s_AA = 1.10, 
                        t_max = 1000) {
  call_freq_a <- c()
  ...
  for (i in 1:n_reps) {
    ...
    # rescue criteria: population outgrows the startpopulation
    if (total_pop > (N_aa_intro+ N_aA + N_AA)) {
      rescue_count <- rescue_count + 1
      freq_a_sum <- (last_gen[1]*2 + last_gen[2]) / (total_pop*2)
      call_freq_a = rbind(call_freq_a, freq_a_sum)
      #print(call_freq_a)
...
print(rescue_prob_aa(20))
# Test "area" of variing number pf introduced N_AA

N_aa_values <- seq(100, 10, by = 10)
#print(N_aa_values)


# Berechne Rescue-Wahrscheinlichkeit pro N_AA
rescue_prob_aas <- sapply(N_aa_values, rescue_prob_aa, n_reps)
#print(rescue_prob_aas)

#Here I transform the returned array (call_freq_a) into a data.frame where we have now 2 columns (1.=N_aa, 2.= allele freq. of a n_reps)

#dim(rescue_prob_aas)
array_rp_aa <- array(NA, dim=c(dim(rescue_prob_aas)[1],2), dimnames = list(c(),c("N_aa","allele.freq_a")))

for(i in N_aa_values){
  
  array_rp_aa[,1] <- i
  
  temp <- rescue_prob_aas[,c(i+1)]
  temp[temp==0] <- NA
  
  array_rp_aa[,2] <- temp
  
  array_rp_aa <- data.frame(array_rp_aa)
  
  if (i==0){ full_array_aa <- array_rp_aa } else { full_array_aa <- rbind(full_array_aa, array_rp_aa)}
}

#plott full_array_aa 
ggplot(full_array_aa, aes(x = N_aa, y = allele.freq_a)) +
  geom_point(aes(color = allele.freq_a), size = 2, alpha = 0.7) +  # dots with color fade
  scale_color_viridis_c(option = "plasma") +                   # fade coloring
  geom_smooth(method = "lm", se = TRUE, color = "darkred", size = 1.2, alpha = 0.3) +  # trendline
  labs(
    title = "Allele Frequencies vs. Time of Introduced Individuals",
    subtitle = "N_AA = 10",
    x = "Time of Introduction (N_AA)",
    y = "Allele Frequency (a)",
    color = "Allele Frequency"
  ) +
  theme_minimal(base_size = 14) +       # clean and big theme
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
    plot.subtitle = element_text(size = 12, hjust = 0.5),
    axis.title = element_text(face = "bold"),
