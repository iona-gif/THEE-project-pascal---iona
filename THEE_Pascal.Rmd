...
Error: if schleife nimmt summe und nicht wert an position n für weitermachen in neuem teil. 
...
#new function to simulate pop size; because of error before -> fix for better understanding

simulate_pop_multi <- function(N_aa, N_aA, N_AA, r_aa, s_aA, s_AA, t_max) {
  # Create the vector in which to save the results
  pop_vector <- c(N_aa, N_aA, N_AA)
  # initiate the variables
  pop_new <- c(N_aa, N_aA, N_AA)
  ...
  for (i in 1:(t_max+1)) {
    ...
    if (pop_new[1]+pop_new[2]+pop_new[3]>=101*(Naa+NAA) | pop_new[1]+pop_new[2]+pop_new[3]==0) break
    #print(pop_new[1]+pop_new[2]+pop_new[3])
    # redefine the current population one generation later
    pop_new <- simulate_gen(pop_new[1],pop_new[2],pop_new[3], r_aa, s_aA, s_AA)
    #print(pop_new)
...
####start of excercise 3

# initialize data table - where to collect the results
data_table <- data.frame(array(NA, dim = c(0,5), dimnames = list(c(),c())))
  ...
  for(NAA in N_AA){
    ...
    # reset counter
      ...
      repeat {
        ...
        one_run <- simulate_pop_multi(N_aa, N_aA, N_AA, r_aa, s_aA, s_AA, t_max)
        
        cat("coment one run:", one_run, "\n")
        # determine total population sizes
        total_size <- one_run[,1]+one_run[,2]
        cat("coment total size:", total_size, "\n")
        # determine minimum population size
        min_size <- min(total_size)
         cat("coment min size:", min_size, "\n")
        # determine (first) generation at which this population size occurred
        min_gen <- as.numeric(which(total_size==min_size)[1])
        cat("coment min gen:", min_gen, "\n")
        # enter the data into the table
        
        data_table <- rbind(data_table,data.frame(c(Naa,NAA,saA,min_gen,min_size))) # note that we add the varying parameters (decay rate and selection coefficient) to the table too
        # stop the repeated computation after no_replicates times
         cat("coment data table:", data_table, "\n")
