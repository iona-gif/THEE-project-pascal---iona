...
N_aa_vector <- c(10,100,50,40)
N_AA_vector <- c(2,5,7,10)
s_aA_vector <- c(1.1,1.2,1,0.9) 
  ...
  for(N_AA in N_AA_vector){
    ...
    for(s_aA in s_aA_vector){
      ...
      repeat {
        ...
        data_table <- rbind(data_table,data.frame(N_aa=N_aa, N_aA=N_aA, N_AA=N_AA,  min_gen = min_gen, min_size = min_size)) # note that we add the varying parameters (decay rate and selection coefficient) to the table too
...
colnames(data_table) <- c("N_aa","N_aA", "s_aA","min_gen","min_size")
