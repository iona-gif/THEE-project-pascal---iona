...
```{r}
# --- Rescue probability simulation (dein Original) ------------------
n_reps <- 100
...
rescue_prob_aa <- function(N_aa_intro, N_AA = 10, n_reps = 100,
  ...
  return(call_freq_a)            # bleibt wie von dir gewünscht
}

# ---------- dein Test-Block -----------------------------------------
N_aa_values <- seq(100, 10, by = -10)   # Reihenfolge beibehalten
rescue_prob_aas <- sapply(N_aa_values, rescue_prob_aa, n_reps)
# rescue_prob_aas ist jetzt ein 100 x length(N_aa_values) - Matrix

# ---------- Aufbereitung (nur Indizierung korrigiert) ---------------
full_list <- lapply(seq_along(N_aa_values), function(idx) {
  n_aa <- N_aa_values[idx]
  vec  <- rescue_prob_aas[, idx]   # korrekte Spalte!
  vec[vec == 0] <- NA
  data.frame(N_aa = n_aa, allele.freq_a = vec)
})
full_array_aa <- do.call(rbind, full_list)
...
ggplot(full_array_aa, aes(x = N_aa, y = allele.freq_a)) +
  ...
  scale_y_continuous(limits = c(0, 0.15), breaks = seq(0, 0.15, by = 0.05)) + # fixe y-Skala
  labs(
    title = "Allele Frequencies vs. Time of Introduced Individuals",
    subtitle = "N_AA = 10",
    x = "Time of Introduction (pop. size N_aa as proxy for time)",
    y = "Allele Frequency (a)",
    color = "Allele Frequency"
  ) +
